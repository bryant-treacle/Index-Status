...
#curl -XGET 'localhost:9200/_cat/health?h=cluster,status,node.total,node.data,shards,pri,relo,init,unassign,pending_tasks,max_task_wait_time,active_shards_percent&format=json'  | tee curl -XPUT -H "content-type: application/json" 'http://localhost:6000/' -d @- 
...
#curl -XGET 'localhost:9600/_node/stats/events' | jq -r '.' | curl -XPUT -H "content-type: application/json" 'http://localhost:6000/' -d @-
#sleep 10
#done
